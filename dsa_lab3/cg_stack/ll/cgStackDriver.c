...
int main(int argc, char *argv[])
{
    ...
    */
	...
	total_push += ((t2.tv_sec - t1.tv_sec)*1e6 + (t2.tv_usec - t1.tv_usec))*1e-6;
        i++;
    }
    fclose(fp);
    ...
    */
    printf("Time taken to push all elements: %lf\nMemory used: %ld\n", total_push, heapMemoryAllocated);
    ...
    */
    ...
    while(pop(s));
    gettimeofday(&t2, NULL);
    emptyStack += ((t2.tv_sec - t1.tv_sec)*1e6 + (t2.tv_usec - t1.tv_usec))*1e-6;
    printf("Time taken to empty all elements: %lf\n", emptyStack);

    /*
        Print the total time taken for the entire process and the maximum heap usage throughout the process
    */
    printf("Time taken to do everything: %lf\n", emptyStack + total_push);
}
Element iftoe (int i, float f)
{
    Element e;
